<#
.SYNOPSIS
  Build and publish the cisa-for-bdsca package, with version bumping and optional install verification.
...
.PARAMETER GitHubRepo
  GitHub repository in owner/repo format. Default: yourusername/cisa-for-bdsca
...
function Set-ProjectVersion {
  ...
  $initPath = Join-Path (Split-Path $PyProjectPath) 'src/cisa_bdsca/__init__.py'
...
function New-GitHubRelease {
  ...
  $headers = @{
    ...
    'User-Agent'  = 'cisa-bdsca-release-script'
  }
  # Build JSON payload
...
try {
  if ($CreateGitHubRelease) {
    ...
    if ($ReleaseNotesPath -and (Test-Path $ReleaseNotesPath)) {
      ...
      $relBody = "Release $tag`n`nPublished to $Repository as cisa-for-bdsca $nextVersion."
    }
...
if ((-not $NoInstallTest) -and (-not $DryRun)) {
  ...
  if (-not $NoUpload) {
    ...
    $interval = 5
    $waited = 0
    $pypiUrl = "https://pypi.org/pypi/cisa-for-bdsca/json"
  ...
  if ($NoUpload) {
    ...
    if ($Repository -eq 'testpypi') {
      $installCmd = "$testPy -m pip install --index-url https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple cisa-for-bdsca==$nextVersion"
    } else {
      $installCmd = "$testPy -m pip install --index-url https://pypi.org/simple/ cisa-for-bdsca==$nextVersion"
    }
  ...
  # Verify CLI
  Invoke-Step -Command "$testPy -m cisa_bdsca.cli --version"
}
