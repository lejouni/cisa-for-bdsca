...
def extract_cisa_data(vuln_response: dict[str, Any]) -> Optional[CISAData]:
    ...
    try:
        ...
        if cisa_obj and isinstance(cisa_obj, dict):
            ...
            logger.debug(f"Found CISA KEV data: addedDate={cisa_obj.get('addedDate')}, dueDate={cisa_obj.get('dueDate')}")
...
def extract_vulnerability_info(
    ...
    try:
        ...
        return VulnerabilityInfo(
            id=vuln_id,
            source=source,
            error=f"Failed to extract vulnerability info: {e}"
        )
