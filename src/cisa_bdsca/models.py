...
class CISAData(BaseModel):
    ...
    date_added: Optional[str] = Field(
        default=None, alias="cisaDateAdded", description="Date added to CISA KEV catalog"
    )

    due_date: Optional[str] = Field(
        default=None, alias="cisaDueDate", description="Remediation due date per CISA directive"
    )

    required_action: Optional[str] = Field(
        default=None, alias="cisaRequiredAction", description="Required action from CISA"
    )

    notes: Optional[str] = Field(
        default=None, alias="cisaNotes", description="Additional notes from CISA"
    )
    ...
    vulnerability_name: Optional[str] = Field(default=None, description="Name of the vulnerability")

    short_description: Optional[str] = Field(
        default=None, description="Short description of the vulnerability"
    )

    known_ransomware_campaign_use: Optional[str] = Field(
        default=None, description="'Known' if used in ransomware campaigns, 'Unknown' otherwise"
    )

    cwes: Optional[list[str]] = Field(default=None, description="Common Weakness Enumeration codes")

    model_config = ConfigDict(populate_by_name=True, extra="allow")  # Allow additional fields
...
class RelatedCVE(BaseModel):
    ...
    cisa_data: Optional[CISAData] = Field(default=None, description="CISA KEV data for this CVE")
...
class VulnerabilityInfo(BaseModel):
    ...
    related_cves: list[RelatedCVE] = Field(
        default_factory=list, description="Related CVEs with their CISA data (for EUVD/BDSA)"
    )
    mapping_source: Optional[MappingSource] = Field(
        default=None, description="Source of CVE mapping for non-CVE vulnerabilities"
    )

    # CISA data (only at top level for direct CVE queries)
    cisa_data: Optional[CISAData] = Field(
        default=None, description="CISA KEV data (only for direct CVE queries)"
    )

    # Error information
    error: Optional[str] = Field(
        default=None, description="Error message if data could not be retrieved"
    )
...
class CollectionResult(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="When the collection was performed"
    )
    ...
    cisa_available_count: int = Field(default=0, description="Number with CISA data available")
    kev_count: int = Field(default=0, description="Number in CISA KEV catalog")
    ...
    def add_error(self, vuln_id: str, error_message: str) -> None:
        ...
        self.errors.append({"vulnerability_id": vuln_id, "error": error_message})
        self.error_count += 1
