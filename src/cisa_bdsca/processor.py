...
def process_vulnerabilities(
    vuln_ids: list[str], config: Optional[Config] = None, use_kev_catalog: bool = False
) -> CollectionResult:
    ...
    result = CollectionResult(
        total_count=len(vuln_ids), success_count=0, error_count=0, vulnerabilities=[]
    )
        ...
        try:
            ...
            if source == VulnerabilitySource.NVD:
                ...
                vuln_info = VulnerabilityInfo(
                    id=vuln_id, source=source, error="Unknown vulnerability source"
                )
            ...
            result.vulnerabilities.append(
                VulnerabilityInfo(
                    id=vuln_id, source=determine_source_from_id(vuln_id), error=str(e)
                )
            )
...
def _process_cve(
    cve_id: str, bd_client: BlackDuckClient, kev_catalog: Optional[KEVCatalog] = None
) -> VulnerabilityInfo:
    ...
    try:
        ...
        vuln_info = extract_vulnerability_info(cve_id, vuln_data, VulnerabilitySource.NVD)
        ...
        return VulnerabilityInfo(
            id=cve_id, source=VulnerabilitySource.NVD, error=f"Black Duck API error: {e}"
        )


def _process_bdsa(
    bdsa_id: str, bd_client: BlackDuckClient, kev_catalog: Optional[KEVCatalog] = None
) -> VulnerabilityInfo:
    ...
    try:
        ...
        vuln_info = extract_vulnerability_info(bdsa_id, vuln_data, VulnerabilitySource.BDSA)
        ...
        for cve_id in related_cve_ids:
            ...
            if cve_data:
                # Extract CVE info
                cve_info = extract_vulnerability_info(cve_id, cve_data, VulnerabilitySource.NVD)
        ...
        return VulnerabilityInfo(
            id=bdsa_id, source=VulnerabilitySource.BDSA, error=f"Black Duck API error: {e}"
        )
...
def _process_euvd(
    ...
    vuln_info = VulnerabilityInfo(id=euvd_id, source=VulnerabilitySource.EUVD)
